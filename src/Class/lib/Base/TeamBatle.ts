...
export class TeamBattle extends BaseBattle {
    ...
    constructor(i: CommandInteraction, teamA: Team, teamB: Team) {
        ...
        this.teamA = {
            ...teamA,
            fighters: teamA.fighters.map((x) => cloneDeep(x)),
        };
        this.teamB = {
            ...teamB,
            fighters: teamB.fighters.map((x) => cloneDeep(x)),
        };
    }
    ...
    async run() {
        ...
        while (
            this.teamA.fighters.length !== 0 &&
            this.teamB.fighters.length !== 0
        ) {
            this.round++;
            ...
            const opponent = random.pick(defendTeam.fighters);
            ...
            for (let i = 0; i < this.fighters.length; i++) {
                ...
                if (i === 0) {
                    battleEmbed.addFields({
                        name: "\u200b",
                        value: `**${this.teamA.name}**`,
                    });
                } else if (i === teamAlength) {
                    battleEmbed.addFields({
                        name: "\u200b",
                        value: `**${this.teamB.name}**`,
                    });
                }
            ...
            if (opponent.hp <= 0) {
                const index = defendTeam.fighters.findIndex(
                    (x) => x.id === opponent.id,
                );
